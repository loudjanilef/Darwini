...
class Breeder:
    ...
    def __initialize(self):
        for i in range(self.population_size):
            ...
            print("Generation {} : Training model {}/{}".format(self.generation_nbr + 1, i + 1, self.population_size))
    ...
    def generation(self):
        ...
        self.generation_nbr += 1
        selected = self.population[:5]
        self.population = self.population[5:]
        selected.extend(random.sample(self.population, 5))
        self.population = selected
        for i, pair in enumerate(combinations(selected, 2)):
